...
export function AuthUI({ mode, signInWithPassword, signUpWithPassword }: AuthUIProps) {
  ...
  return (
    <section className="mt-16 flex w-full flex-col gap-16 rounded-2xl border border-border bg-card p-10 px-4 text-center shadow-xl">
      <div className="flex flex-col gap-4">
        <Image src="/logo.png" width={80} height={80} alt="" className="m-auto" />
        <h1 className="text-lg text-foreground">{titleMap[mode]}</h1>
      </div>
      <div className="flex flex-col gap-4">
        <form
          className="flex flex-col gap-4 rounded-xl border border-border bg-background p-8 text-left shadow-sm"
          ...
          {isSignup ? (
            <label className="flex items-start gap-2 text-left text-sm text-muted-foreground">
              <input
                type="checkbox"
                name="acceptTerms"
                className="mt-1 h-4 w-4 shrink-0 rounded border border-input bg-background accent-primary"
                required
                disabled={pending}
              />
              <span>
      ...
      {mode === "login" ? (
        <span className="m-auto max-w-sm text-sm text-muted-foreground">
